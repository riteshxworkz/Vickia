...
<body>
    ...
    <script type="text/javascript">
        ...
        checkoutButton.addEventListener('click', async function () {
            ...
            // created in step 3.
           var stripeResponse=  fetch('{{ route('user.payments.stripe.process') }}', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                    },
                })
               var result = await stripeResponse.then(function (response) {
                ...
                // stripeResponse.then(await function (session) {
                //     return stripe.redirectToCheckout({
                //         sessionId: session.id
                //     });
                // })

                stripeResponse.then(function(result) {
                    console.log(result);
