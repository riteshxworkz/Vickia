				...
				<div class="card-body pt-5">
					<form class="w-100" action="{{ route('user.templates.custom.store') }}" method="POST" enctype="multipart/form-data">
						...
						<div class="row">
							...
							<div class="col-lg-6 col-md-12 col-sm-12">
								<div class="input-box">
									<h6>{{ _('Template Icon') }} <span class="text-required"><i class="fa-solid fa-asterisk"></i></span><i class="ml-3 text-dark fs-13 fa-solid fa-circle-info" data-tippy-content="{{ _('You will need to get it from fontawesome.com') }}"></i></h6>
									<div class="form-group">
							...
							<div class="col-sm-12 mt-4 mb-5">
								<div class="form-group">
									...
									<div class="field input-group mb-4">
										...
										<input type="text" class="form-control" name="placeholders[]" placeholder="{{ _('Enter Input Field Description') }} ({{ _('Required') }})">
...
@section('js')
	...
	<script type="text/javascript">
		$(function () {
			...
			let table = $('#allTemplates').DataTable({
				...
				language: {
					...
					"info": "{{ _('Showing page') }} _PAGE {{ _('of') }} _PAGES",
					...
					if (result.value) {
						...
						$.ajax({
							...
							success: function (data) {
								if (data == 'success') {
									Swal.fire('{{ _('Description Update') }}', '{{ _('Description has been successfully updated') }}', 'success');
									$("#allTemplates").DataTable().ajax.reload();
								} else {
									Swal.fire('{{ _('Update Error') }}', '{{ _('Description was not updated correctly') }}', 'error');
								}
							},
							error: function(data) {
						...
						Swal.fire('{{ _('No Description Entered') }}', '{{ _('Make sure to provide a new description before updating') }}', 'error')
					}
				})
			});
			...
			$(document).on('click', '.activateButton', function(e) {
				...
				$.ajax({
					...
					success: function (data) {
						if (data == 'success') {
							Swal.fire('{{ _('Template Activated') }}', '{{ _('Template has been activated successfully') }}', 'success');
							$("#allTemplates").DataTable().ajax.reload();
						} else {
							Swal.fire('{{ _('Template Already Active') }}', '{{ _('Selected template is already activated') }}', 'warning');
						}
					},
					error: function(data) {
			...
			$(document).on('click', '.deactivateButton', function(e) {
				...
				$.ajax({
					...
					success: function (data) {
						if (data == 'success') {
							Swal.fire('{{ _('Template Deactivated') }}', '{{ _('Template has been deactivated successfully') }}', 'success');
							$("#allTemplates").DataTable().ajax.reload();
						} else {
							Swal.fire('{{ _('Template Already Deactive') }}', '{{ _('Template is already deactivated') }}', 'warning');
						}
					},
					error: function(data) {
						Swal.fire({ type: 'error', title: 'Oops...', text: 'Something went wrong!' })
				...
				Swal.fire({
					...
					if (result.isConfirmed) {
						...
						$.ajax({
							...
							success: function (data) {
								if (data == 'success') {
									Swal.fire('{{_('Custom Template Deleted')}}', '{{ _('Custom template has been successfully deleted') }}', 'success');
									$("#allTemplates").DataTable().ajax.reload();
								} else {
									Swal.fire('{{ _('Template Delete Failed') }}', '{{ _('There was an error while deleting this custom template') }}', 'error');
								}
							},
							error: function(data) {
								Swal.fire({ type: 'error', title: 'Oops...', text: 'Something went wrong!' })
		...
		function addField(plusElement){
			...
			let new_field ='<div class="field input-group mb-4">' +
								...
								'<input type="text" class="form-control" placeholder="{{ _('Enter Input Field Description') }} ({{ _('Required') }})" name="placeholders[]">' +
								'<select class="form-select mr-4" name="input_field[]" onchange="notifyUser(this)">' +
									'<option value="input" selected>{{ __('Input Field') }}</option>' +
									'<option value="textarea">{{ __('Textarea Field') }}</option>' +
									'<option value="select">{{ __('Select List Field') }}</option>' +
    ...
    try {
        // Traverse to the root element of the field block (assuming it's a div or similar container)
        let fieldContainer = minusElement.closest('.field');

        if (!fieldContainer) throw new Error("Field container not found");

        // Extract the ID from the field name (or some other attribute, if necessary)
        let inputField = fieldContainer.querySelector('input[name="names[]"]');
        let fieldId = inputField.id; // This is the ID created in addField like `input-field-x`

        // Remove the corresponding button
        deleteButton(fieldId);
