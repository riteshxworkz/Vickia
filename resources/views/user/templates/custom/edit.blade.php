						...
						<div class="row">
							<div class="col-sm-12 mt-4 mb-5">
								<div class="form-group">
									...
									@endforeach
									{{-- <div class="field input-group mb-4">
										<input type="hidden" name="code[]" value="input-field-{{ count($id->fields) + 1 }}">
										<input type="text" class="form-control" name="names[]" placeholder="{{ __('Enter Input Field Title (Required)') }}" id="input-field-{{ $key + 1 }}" value="">
										@if ($value['input'] == 'input' || $value['input'] == 'textarea')
											<input type="text" class="form-control" name="placeholders[]" placeholder="{{ __('Enter Input Field Description') }}" value="">
										@else
											 <input type="text" class="form-control" name="placeholders[]" placeholder="@foreach ($value['placeholder'] as $option) {{ $option }}, @endforeach" value="">
										@endif
										<select class="form-select mr-4" name="input_field[]" onchange="notifyUser(this)">
										...
										<span onclick="addField(this, {{ count($id->fields) + 1 }})" class="btn btn-primary">
											<i class="fa fa-btn fa-plus"></i>
										</span>
										<span onclick="removeField(this)" class="btn btn-primary">
											<i class="fa fa-btn fa-minus"></i>
								...
								<div class="input-box">
									...
									<div class="form-group">
										<div id="field-buttons">
											@foreach ($id->fields as $key=>$value)
												<span onclick="insertText(this)" id="'input-field-{{ $key + 1 }}'-button" class="btn btn-primary mr-4 mb-2">input-field-{{ $key + 1 }}</span>
											@endforeach
										</div>
...
@section('js')
	...
	<script type="text/javascript">
		...
		function addField(plusElement, k){
			...
			createButton(increment_alphanumeric_str(name.id));
