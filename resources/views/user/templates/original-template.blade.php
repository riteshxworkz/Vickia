					...
					<div class="row">
						...
						<div class="col-sm-12">
							<div class="input-box mb-4">
								<div class="form-group">
									<label class="custom-switch mb-0">
										<input type="checkbox" id="file-upload-switch" name="file-upload-switch" class="custom-switch-input">
										<span class="custom-switch-indicator"></span>
										<span class="custom-switch-description">{{ __('Upload a file?') }}</span>
									</label>
								</div>
							</div>
						</div>

						<div class="col-sm-12 file-upload" style="display: none;">
							<!-- DRAG & DROP FILES -->
...
<script type="text/javascript">
	...
	$(function () {
		...
		FilePond.setOptions({
			maxFileSize: 25 + 'MB',
			maxFiles: 1,
			labelIdle: "{{ __('Drag & Drop your file or') }} <span class=\"filepond--label-action\">{{ __('Browse') }}</span><br><span class='restrictions'><span class='restrictions-highlight'>.pdf, .doc, .csv</span></span><br><span class='restrictions'><span class='text-muted'>{{ __('Max File Size') }}: " + 25 + "MB</span></span>",
			required: false,
			instantUpload:true,
			storeAsFile: true,
		...
		async function uploadFile(formData) {
			try {
				const response = await fetch('/user/chat/file/store', {
					method: 'POST',
					headers: {
						'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
					},
					body: formData,
					// fetch automatically handles processData and contentType for FormData
				});
		...
		// SUBMIT FORM
		$('#openai-form').on('submit', async function(e) {
			...
			if (pond.getFiles().length !== 0) {
				...
				if(files.length > 0) {
					...
					collection_id = await uploadFile(formData)
					formSerial += '&embed='+collection_id
				}
			}
			...
			$.ajax({
				...
				success: async (data) => {
					if (data['status'] == 'error') {
						...
						var path = "/user/templates/original-template/process?content_id=" + data.id+"&max_results=" + data.max_results + "&max_words=" + data.max_words + "&temperature=" + data.temperature + "&language=" + data.language;
						if(files.length > 0) {
							path += path +'&embed='+collection_id
						}
	...
	let fileCheckbox = document.getElementById('file-upload-switch');

	if (fileCheckbox) {
		fileCheckbox.addEventListener('change', (event) => {
			if (event.currentTarget.checked) {
				let myElements = document.querySelectorAll(".file-upload");
