	...
	*/
    ...
    $user_plan_id = auth()->user()->plan_id;
       ...
       $user_id = auth()->user()->id;
        return view('user.chat.view', compact('chat', 'messages', 'categories', 'prompts', 'internet' , 'user_id', 'user_plan_id'));
	}


	public function  publicChat($chatCode,$userId,$internet,$user_plan_id = null){
     
        $chat = Chat::where('chat_code', $chatCode)->first(); 
        $messages = ChatConversation::where('user_id',$userId)->where('chat_code', $chat->chat_code)->orderBy('updated_at', 'desc')->get(); 
        $categories = ChatPrompt::where('status', true)->groupBy('group')->pluck('group'); 
        $prompts = ChatPrompt::where('status', true)->get();
        
        if (!is_null($user_plan_id)) {
            $plan = SubscriptionPlan::where('id', $user_plan_id)->first();
            $internet = $plan->internet_feature;
        } else {
            if ($internet && isset($internet) && !empty($internet) ) {
                $internet = true;
            } else {
                $internet = false;
            }
         ...
         return view('user.chat.chat-view', compact('chat', 'messages', 'categories', 'prompts', 'internet' , 'user_id'  ));
  }
